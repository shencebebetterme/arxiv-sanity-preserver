...
#limiter = Limiter(app, application_limits=["100 per hour", "20 per minute"])
...
def papers_from_library():
  #print("calling papers_from_library")
  out = []
  if g.user:
...
def search():
  ...
  # pass what is typed in html's searchbox to a new variable q
...
def review():
  ...
  # correspond to $.post("/libtoggle", {pid: pid}) line in .js
  ...
  if record:
    ...
    #do not return a template html here, so creating a null page to handle library
    ret = 'ON'
